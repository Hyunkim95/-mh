...
**Team:** continue-orchestrator-end-to-e
**Role:** executor
**Worker Name:** worker-3

## Your Assigned Tasks

- **Task 3**: Review and document: Continue orchestrator end-to-end integration. Split into: b
  Description: Review code quality and update documentation for: Continue orchestrator end-to-end integration. Split into: backend route/API migration off obfuscation, frontend deploy/status migration off obfuscation, and verification/regression. Keep backend and frontend lifecycle aligned. Do not revert unrelated dirty files. Verify before shutdown.
  Status: pending
  Role: executor
...
1. Load and follow the worker skill from the first existing path:
   ...
   - `/Users/brotherofalphakim/dev/multihopper/.codex/skills/worker/SKILL.md`
   - `/Users/brotherofalphakim/dev/multihopper/skills/worker/SKILL.md` (repo fallback)
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):

   `omx team api send-message --input "{"team_name":"continue-orchestrator-end-to-e","from_worker":"worker-3","to_worker":"leader-fixed","body":"ACK: worker-3 initialized"}" --json`
...
5. Read the task file for your selected task id at `/Users/brotherofalphakim/dev/multihopper/.omx/state/team/continue-orchestrator-end-to-e/tasks/task-<id>.json` (example: `task-1.json`)
6. Task id format:
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/Users/brotherofalphakim/dev/multihopper/.omx/state/team/continue-orchestrator-end-to-e/workers/worker-3/status.json`
13. Wait for the next instruction from the lead
14. For legacy team_* MCP tools (hard-deprecated), use `omx team api`; do not pass `workingDirectory` unless the lead explicitly asks (if resolution fails, use leader cwd: `/Users/brotherofalphakim/dev/multihopper`)

## Mailbox Delivery Protocol (Required)
When you are notified about mailbox messages, always follow this exact flow:

1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"continue-orchestrator-end-to-e","worker":"worker-3"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"continue-orchestrator-end-to-e","worker":"worker-3","message_id":"<MESSAGE_ID>"}" --json`
...
Example: omx team api send-message --input "{"team_name":"continue-orchestrator-end-to-e","from_worker":"worker-3","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
