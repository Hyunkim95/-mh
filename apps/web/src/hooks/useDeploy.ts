...
export const useDeploy = () => {
  ...
  const deploy = async (data: {
    ...
    try {
      ...
      toast.success(`${type} Route deployed successfully! Signature: ${signature.slice(0, 8)}...`, { id: 'deploy' });
      console.log('Route deployed with signature:', signature);
      
      return signature;
    } catch (error) {
      console.error(`${type} Route deployment failed:`, error);
      toast.error(`${type} Route deployment failed: ${error instanceof Error ? error.message : 'Unknown error'}`, { id: 'deploy' });
      throw error;
    }
  };
