...
export function useExecutor(routeId: number) {
  ...
  return {
    ...
    // Balance information
    balance: balanceQuery.data?.data?.balance ? new BN(balanceQuery.data.data.balance) : null,
    balanceSOL: balanceQuery.data?.data?.balanceSOL ? parseFloat(balanceQuery.data.data.balanceSOL) : null,
