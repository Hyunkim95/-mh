...
const balance = async (routeId: number): Promise<BN> => {
    try {
        ...
        console.error('Error getting balance for route ID', routeId, 'error', error);
        throw error;
    }
};
 ...
 */
const withdrawOnBehalf = async (routeId: number, to: string, amount: BN): Promise<string> => {
    try {
        ...
        if (currentBalance.lt(amount)) {
            throw new Error(`Insufficient balance. Current: ${currentBalance.toString()}, Required: ${amount.toString()}`);
        }
        ...
        console.error('Error withdrawing on behalf for route ID', routeId, 'error', error);
        throw error;
    }
};
