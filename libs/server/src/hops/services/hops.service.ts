...
const getHopById = async (hopId: number): Promise<Hop | null> => {
  const [hop] = await db
    .select()
    .from(hopsSchema)
...
const getHopByRouteAndIndex = async (
  routeId: number,
  hopIndex: number
): Promise<Hop | null> => {
  const [hop] = await db
    ...
    .from(hopsSchema)
    .where(
      and(eq(hopsSchema.routeId, routeId), eq(hopsSchema.hopIndex, hopIndex))
    );
  return hop || null;
};

// Create multiple timestamp-based hops for a route
const createHopsForRoute = async (
  routeId: number,
  hopsData: Omit<CreateHopInput, "routeId">[]
): Promise<Hop[]> => {
...
const getHopsInTimeRange = async (
  startTime: Date,
  endTime: Date,
  routeId?: number
): Promise<Hop[]> => {
