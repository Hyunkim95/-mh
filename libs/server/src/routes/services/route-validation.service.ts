...
import { getTokenConfigSPL, getTokenConfigSOL } from '../../solana/contract.service';
 ...
 */
export async function validateRouteAgainstTokenConfig(routeInput: RouteInput): Promise<ValidationResult> {
  const errors: string[] = [];
  ...
  try {
    ...
    if (hopCount > maxHops) {
      errors.push(`Route has ${hopCount} hops but token config allows maximum ${maxHops} hops`);
    }
    ...
    if (hopAmountRaw < minTransfer) {
      errors.push(`Hop amount ${hopAmountRaw} is below minimum transfer amount ${minTransfer}`);
    }
    ...
    for (let i = 1; i < routeInput.hops.length; i++) {
      ...
      const previousHopTime = new Date(routeInput.hops[i - 1].scheduledAt).getTime();
      const delaySeconds = Math.floor((currentHopTime - previousHopTime) / 1000);
      
      if (delaySeconds > maxDelaySeconds) {
        errors.push(`Delay between hop ${i} and hop ${i + 1} is ${delaySeconds} seconds, which exceeds maximum allowed delay of ${maxDelaySeconds} seconds`);
      }
      
      if (delaySeconds < 0) {
        errors.push(`Hop ${i + 1} is scheduled before hop ${i}. Hops must be scheduled in chronological order`);
      }
    }
    ...
    if (routeInput.hops.length > 0) {
      ...
      if (delayToFirstHop > maxDelaySeconds) {
        errors.push(`First hop is scheduled ${delayToFirstHop} seconds in the future, which exceeds maximum allowed delay of ${maxDelaySeconds} seconds`);
      }
    }
    ...
    errors.push('Failed to validate route: Unable to fetch token configuration');
    return { isValid: false, errors };
  }
}
 ...
 */
export function validateHopRecipients(hops: Array<{ recipient: string }>): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateHopScheduling(hops: Array<{ scheduledAt: string }>): ValidationResult {
  const errors: string[] = [];
  const now = Date.now();
  ...
  for (let i = 0; i < hops.length; i++) {
    ...
    try {
      ...
      if (isNaN(scheduledTime.getTime())) {
        errors.push(`Hop ${i + 1} has invalid scheduled time: ${hop.scheduledAt}`);
        continue;
      }
      ...
      if (scheduledTime.getTime() < oneMinuteAgo) {
        errors.push(`Hop ${i + 1} is scheduled in the past: ${hop.scheduledAt}`);
      }
      ...
      errors.push(`Hop ${i + 1} has invalid scheduled time format: ${hop.scheduledAt}`);
    }
  }
 ...
 */
export async function validateRoute(routeInput: RouteInput): Promise<ValidationResult> {
  ...
  const tokenConfigValidation = await validateRouteAgainstTokenConfig(routeInput);
