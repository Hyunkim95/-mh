...
export const getRouteIdFromPda = async (
...
export const verifyRoutePda = async (
  routePda: PublicKey,
  expectedRouteId: number,
  programId?: PublicKey
): Promise<boolean> => {
  try {
    const derivedPda = await getRouteConfigPda(
      new BN(expectedRouteId),
      programId
    );
...
export const getRouteConfigPdas = async (
  routeIds: number[],
  programId?: PublicKey
): Promise<{ routeId: number; pda: PublicKey }[]> => {
  const results = [];
...
export const isValidRouteConfigPda = async (
  pda: PublicKey,
  programId?: PublicKey
): Promise<boolean> => {
  try {
